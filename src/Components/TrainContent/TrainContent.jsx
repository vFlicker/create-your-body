...
export default function TrainContent({ view, level, base }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const weeksResponse = await axios.get(`${API_BASE_URL}/cms/api/workouts/client-weeks`);
                ...
                for (const week of weeks) {
                    try {
                        ...
                        const filteredTrainings = response.data.data.filter(
                            training => training.type === view && training.level === mappedLevel
                        );
                        ...
                        console.error(`Ошибка при получении данных для недели ${week.week}:`, error.message);
                    }
                }
    ...
    useEffect(() => {
        const fetchWeekTrainings = async () => {
            ...
            try {
                ...
                console.log(`Получены тренировки для недели ${selectedWeek.week}:`, filteredTrainings);
                ...
                for (const training of filteredTrainings) {
                    try {
                        ...
                        console.error(`Ошибка при получении деталей тренировки ${training._id}:`, error.message);
                        details[training._id] = {
                            stepsCount: 0,
                            coverUrl: null,
                            ...training
                ...
                console.error(`Ошибка при получении тренировок для недели ${selectedWeek.week}:`, error.message);
            } finally {
    ...
    const handleTrainingSelect = async (training) => {
        try {
            ...
            console.error(`Ошибка при получении данных тренировки ${training._id}:`, error.message);
        }
    };
            ...
            <AnimatePresence initial={false} custom={direction}>
                {page === 0 && (
                    <motion.div
                        ...
                        exit="exit"
                        transition={transition}
                        style={{ transition: 'all 0.6s ease-in-out', gridTemplateColumns: isLoading ? '1fr' : '', height: isLoading ? '100%' : '' }}
                    >
                        {isLoading ? (
                            <Loader />
                        ) : (
                                ...
                                return (
                                    <TrainBox
                                        ...
                                        data={{
                                            ...
                                            coverUrl: weekData ? weekDetails[weekNumber]?.coverUrl : null
                                        }}
                                        isClosed={!weekData}
                ...
                {page === 1 && selectedWeek && (
                    <motion.div
                        ...
                        exit="exit"
                        transition={transition}
                        style={{transition: 'all 0.3s ease-in-out', height: isLoadingTrainings ? '100%' : ''}}
                    >
                        <div className="trainings-grid" style={{ transition: 'all 0.3s ease-in-out', gridTemplateColumns: isLoadingTrainings ? '1fr' : '', height: isLoadingTrainings ? '100%' : '' }}>
                            {isLoadingTrainings ? (
                                <Loader />
                            ) : (
